...
@Slf4j
public class FooterXpath extends ActionMethods {
	...
	public FooterXpath(WebDriver driver) {
		super(driver);
		this.driver = driver;
	...
	public boolean validateFooterXpath() {
		try {
				...
				if (validateQuickLinksFooter())
					if (validateContactInfoFooter())
						if (validateLocationFooter())
							if (elementDisplay(txtCopyright))
			...
			Log.info("validation of footer if failed " + e);
			Log.error(e);
		}
		return false;
	}
	...
	public boolean validateAboutCompanyFooter() {
		...
		try {
			if (elementDisplay(txtAboutCompany))
	...
	public boolean validateQuickLinksFooter() {
		try {
			if (elementDisplay(txtQuickLinks))
				if (validateHome())
					if (validateAbout())
	...
	public boolean validateLocationFooter() {
		try {
			...
			if (elementDisplay(txtLocation)) {
				if (elementDisplay(map)) {
